...
% \textbf{} macro for emphasis. Do not put things in all caps. Do not underline
% them. Do not italicize them. Just use bold.
...
\documentclass[14pt,letterpaper,article,oneside]{memoir}

% LaTeX Packages: These expand the functionality of LaTeX. What each one does is
% described after the command
\usepackage{graphicx}  % WITR Graphics
\usepackage{eso-pic}   % Background images
\usepackage{textpos}   % Literally just to line up the date on the cover page
\usepackage{fontspec}  % WITR Fonts
\usepackage{color}     % Colorful text (also for the title page)
\usepackage{hyperref}  % Links to webpages and to email addresses
% \usepackage[showframe,pass]{geometry}  % Show me where the page borders are
...
% Memoir versions
\settrims{0pt}{0pt}
\settypeblocksize{620pt}{350pt}{*} % Set the size of the text block
\setlrmargins{*}{*}{1} % Set the right and left margins
\setulmargins{*}{*}{1.8} % Set the top and bottom margins
...
% Rather than defining a new font, abuse LaTeX's font names a little. Century
% Gothic is techincally a sans serif font, but it's also the main font of the
% document...
\defaultfontfeatures{Mapping=tex-text,Scale=MatchLowercase}
\setmainfont{Century Gothic}
% Bebas Neue is also a sans serif font, but here I'm abusing it as the alternate
% font for the title on the TOC page.
\setsansfont{Bebas Neue}

% Color of the date on the first page. I don't remember why I called it revision
% blue.
...
% The revision date of the manual. Use all caps.
\date{SPRING 2018}
% E-board Names
\newcommand{\wGM}{Steve DaSilva}
\newcommand{\wPD}{Cristoph Grosse}
\newcommand{\wCE}{Ben Reynolds}
\newcommand{\wEC}{Nick Mailhot}
\newcommand{\wMAL}{Cole Johnson}
\newcommand{\wBD}{Jimmy Hermann}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Paragraph Geometry Settings %
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\setlength{\parindent}{0cm} % Don't indent paragraphs
\setlength{\parskip}{0.9em} % Skip 0.9 times the height of a line after each
                            % paragraph

% Avoid hyphenating things
\hyphenpenalty=7500
...
\begin{center}
    ...
    \includegraphics[width=2cm,height=2cm]{images/witrlogo} \\
    ...
    \parbox[c][3.6cm][c]{\textwidth}{%
        ...
        % anyway.
        \centering
        \fontsize{2.7cm}{1.5cm}\selectfont \textsf{MEMBER TRAINING\\ MANUAL}
    } 
...
% Add a background image to every page from here on out. Note the difference
% between this and the previous commands is the lack of asterisk
\AddToShipoutPictureBG{\BodyBG}

% Again, workaround the space being ignored
\mbox{}
\vspace{1cm}
...
\begin{skinnyitemize}
    \bolditem{WITR Office Number}{(585) 475-2000}
    \bolditem{WITR Request Line}{(585) 475-2271}
...
% href is the command that makes that into a clickable hyperlink. The first
...
\begin{enumerate}
    ...
    \item Obscene material may never be played on air at any time. FCC laws
        ...
        % Negative spaces are a thing. This moves it up 4 mm
        \vspace{-4mm}
...
the officer.  discretion. Members can be suspended for 1\textsuperscript{st}
time offenses.
...
% Skip half a centimeter down, for visual harmony
\vspace{0.5cm}
